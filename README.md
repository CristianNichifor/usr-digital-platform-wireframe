...
Suita completa de 15 teste ruleaza in Chromium; cele doua teste de contacte si cele doua din `tests/resource-filters.spec.ts` ruleaza suplimentar in Firefox si WebKit (23 executii in total). Filtrele de resurse folosesc Civic UI v0.1.0 pentru cautare, continut si organizatie; restul controalelor din ResourceHub raman nemigrate. Testele verifica filtre combinate, rezultate goale, resetare, etichete exacte, focus si padding. Contactele si filtrele de resurse sunt verificate si la 320px. Pentru un singur motor: `npm test -- --project=firefox`. `DEMO_CHROMIUM` afecteaza numai proiectul Chromium.
...
- [src/components/civic/README.md](src/components/civic/README.md): adoptarea Civic UI v0.1.0 din GitHub Releases pentru Contacte publice si randul de filtre pentru resurse.
