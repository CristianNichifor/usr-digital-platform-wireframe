...
const facts = [
  {
    id: 'buget',
    title: 'Execuție bugetară locală',
    figure: '31%',
    source: 'ForExeBug / Trezorerie',
    claim: (county: string) => `${county}: investițiile plătite sunt la nivel sintetic de 31% din programul anual.`,
    variants: {
      direct: 'În Județul Model, investițiile plătite sunt la 31% din programul anual. Sursa este ForExeBug / Trezorerie, verificată la 08 septembrie 2026.',
      local: 'În localitatea mea, asta se vede în proiecte începute târziu. Datele sintetice indică 31% investiții plătite în Județul Model.',
      tehnic: 'Indicator sintetic: execuție investiții 31%, sursă ForExeBug / Trezorerie, verificare 08.09.2026. Exemplul local trebuie completat manual.',
    } as Record<string, string>,
  },
  {
    id: 'cv',
    title: 'CV-uri publicate',
    figure: '2 din 4',
    source: 'director sintetic',
    claim: (county: string) => `${county}: jumătate dintre aleșii din eșantion au CV publicat în machetă.`,
    variants: {
      direct: 'În eșantionul pentru Județul Model, 2 din 4 aleși au CV publicat. Lipsurile trebuie afișate, nu ascunse.',
      local: 'Când CV-ul lipsește, oamenii trebuie să vadă asta clar. Exemplul din Județul Model arată 2 din 4 CV-uri publicate.',
...
function MemberZoneScreen({ route }: { route: HashRoute }) {
  const county = route.params.get('judet') ?? 'Județul Model';
  ...
  const text = route.params.get('text') ?? defaultText;

  function updateMemberParams(values: Record<string, string | null>) {
    const next = new URLSearchParams(route.params);
    Object.entries(values).forEach(([key, value]) => {
      if (!value || value === 'toate') next.delete(key);
      else next.set(key, value);
    });
    replaceHash('/implica-te', next);
  }

  async function copyShareText() {
    await navigator.clipboard?.writeText(text);
    updateMemberParams({ copiat: 'da' });
  }

  return (
    <div className="stack">
      <section className="page-heading">
        <p className="eyebrow">Zona membrilor</p>
        <h1>Ce pot face acum</h1>
        <p>Vedere autentificată simulată. Nu există login real. Nu există trimitere automată.</p>
      </section>

      <section className="wire-section">
        <h2>CE POT FACE ACUM</h2>
        <div className="task-grid">
          {[
            ['Verificați CV-uri lipsă', 'Rezultat: listă de 2 aleși fără CV', 'Owner: secretar filială'],
            ['Pregătiți întrebări pentru adunare', 'Rezultat: 4 întrebări pentru buget local', 'Owner: coordonator politici'],
            ['Documentați o hotărâre locală', 'Rezultat: fișă HCL cu sursă și voturi', 'Owner: grup administrație'],
            ['Verificați un proiect normativ', 'Rezultat: 2 termene și 1 termen nedefinit', 'Owner: echipă legislativă'],
            ['Ajutați la completitudinea UAT', 'Rezultat: 3 linkuri sursă verificate', 'Owner: voluntar date'],
          ].map(([title, outcome, owner]) => (
      ...
      <section className="operations-layout" aria-labelledby="operations-title">
        <div className="wire-section">
          <p className="eyebrow">Online și teren</p>
          <h2 id="operations-title">Plan de lucru al filialei</h2>
          <SourceFlow
            items={[
              'alegeți tema locală',
              'verificați cifra și sursa',
              'descărcați materialul',
              'organizați întâlnirea',
              'marcați rezultatul',
            ]}
          />
        </div>
        <div className="wire-section">
          <p className="eyebrow">Materiale pregătite</p>
          <MetricGrid
            items={[
              {
                label: 'Întâlniri propuse',
                value: '3',
                source: 'calendar filială sintetic',
                note: 'Fiecare are temă, loc, owner și material.',
              },
              {
                label: 'Pachete teren',
                value: '6',
                source: 'bibliotecă materiale sintetică',
                note: 'Fișe locale, grafice, întrebări și variantă de text.',
              },
              {
                label: 'Voluntari potriviți',
                value: '12',
                source: 'profil competențe sintetic',
                note: 'Potrivirea folosește interese declarate, nu profilare ascunsă.',
              },
            ]}
          />
        </div>
      ...
      <section className="wire-section">
        <h2>Fapte de azi</h2>
        <p>Filtrare pe județ. Membrul copiază, editează și publică manual. Nu există auto-post.</p>
        <div className="filter-row" aria-label="Filtru județ fapte">
          {['Județul Model', 'Județul Exemplu', 'Bacău'].map((value) => (
            <a
              key={value}
              href={makeHash('/implica-te', { judet: value, fapt: fact, variant, consimtamant: consent })}
              aria-current={county === value ? 'true' : undefined}
            >
              {value}
            </a>
        ...
        <div className="share-layout">
          <article className="claim-card">
            <h3>{item.title}</h3>
            <p>{item.claim(county)}</p>
            <p>
              Cifră de bază: <Fake source={item.source}>{item.figure}</Fake>
            </p>
            <a className="text-button" href={makeHash('/implica-te', { judet: county, fapt: item.id, variant, export: 'png' })}>
              PNG chart
            </a>
          </article>
          <div>
            <h3>Variante de text</h3>
            <div className="filter-row" aria-label="Registru text">
              {Object.keys(item.variants).map((key) => (
                <a
                  ...
                  href={makeHash('/implica-te', {
                    ...
                    variant: key,
                    consimtamant: consent,
                    text: item.variants[key],
                  })}
                  aria-current={variant === key ? 'true' : undefined}
                >
                  {key}
                </a>
              ))}
            ...
            <textarea
              id="share-text"
              value={text}
              onChange={(event) => updateMemberParams({ text: event.target.value, copiat: null })}
            />
            <p className="prompt">Adaugă un exemplu din localitatea ta.</p>
            ...
            {copied ? <p className="state-note">Text copiat în clipboard. Publicarea rămâne manuală.</p> : null}
          </div>
        </div>
      </section>
      ...
      <section className="wire-section">
        <h2>Colegii mei</h2>
        <div className="filter-row" aria-label="Consimțământ director colegi">
          <a
            href={makeHash('/implica-te', { judet: county, fapt: fact, variant, consimtamant: 'da', text })}
            aria-current={consent === 'da' ? 'true' : undefined}
          >
            apari în director: da
          </a>
          <a
            href={makeHash('/implica-te', { judet: county, fapt: fact, variant, consimtamant: 'nu', text })}
            aria-current={consent === 'nu' ? 'true' : undefined}
          >
            apari în director: nu
          </a>
        </div>
        <div className="consent-grid">
          <article className={consent === 'da' ? 'consent-card active' : 'consent-card'}>
            <h3>Stare da</h3>
            <p>Membru Exemplu apare cu domeniu de interes și rută de contact internă.</p>
          </article>
          <article className={consent === 'nu' ? 'consent-card active' : 'consent-card'}>
            <h3>Stare nu</h3>
            <p>Membru Exemplu nu apare în director. Starea de consimțământ rămâne administrată.</p>
          </article>
        </div>
      </section>
      ...
      <section className="split-layout">
        <div className="wire-section">
          <h2>Procese-verbale filială</h2>
          <ul className="plain-list">
            <li>
              Ședință birou filială, <Fake source="e-USR sintetic">03 iunie 2026</Fake>
            </li>
            <li>
              Adunare generală ordinară, <Fake source="e-USR sintetic">21 aprilie 2026</Fake>
            </li>
          </ul>
        </div>
        <div className="wire-section">
          <h2>Următoarea adunare</h2>
          <p>
            Data propusă: <Fake source="e-USR sintetic">18 septembrie 2026</Fake>
          </p>
          <p>Owner: secretar filială. Rezultat așteptat: proces-verbal publicat intern.</p>
        </div>
      </section>

      <section className="wire-section" aria-labelledby="member-source-title">
        <p className="eyebrow">Unelte interne</p>
        <h2 id="member-source-title">Ce repo-uri pot alimenta zona membrilor</h2>
        <div className="module-list">
          {repoModules
            .filter((module) => ['legislativ', 'romania-reforms'].includes(module.repo))
            .map((module) => (
              <article className="module-card" key={module.repo}>
                <h3>{module.label}</h3>
                <p>{module.prototypeUse}</p>
